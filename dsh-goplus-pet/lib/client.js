/**
 * ============================================================================
 * dsh-goplus-pet 浏览器半侧（browser half）—— 宠物插件的"前端"部分
 * ============================================================================
 *
 * 【这个文件是什么】
 *   本文件是宠物在浏览器里运行的代码。它：
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ============================================================================
		// 说明：
		// - .dsh-goplus-pet-root       宠物的根容器，fixed 定位（相对视口），默认右下角
		// - .dsh-goplus-pet-stage      内部舞台，承载两个 video 的层叠
		// - .dsh-goplus-pet-video      动画视频；opacity 默认 0（隐藏），.is-front 时显示
		// - 双 video 层叠：一个显示、一个预加载，切换时交叉淡入避免闪空白
		const css = [
			// 根容器：fixed 固定定位、层级 40（在界面之上）、整体点击穿透（不挡界面操作）、禁止选中
			'.dsh-goplus-pet-root{position:fixed;z-index:40;pointer-events:none;user-select:none}',
			// 右下角默认位置（right:24px 距右缘、bottom:0 贴底）
			'.dsh-goplus-pet-root[data-corner="bottom-right"]{right:24px;bottom:0}',
			// 左下角位置
			'.dsh-goplus-pet-root[data-corner="bottom-left"]{left:24px;bottom:0}',
			// 舞台：正方形（尺寸由 --dsh-goplus-pet-size 控制，默认 260px），本身不响应鼠标
			'.dsh-goplus-pet-stage{position:relative;width:var(--dsh-goplus-pet-size,260px);height:var(--dsh-goplus-pet-size,260px);pointer-events:none}',
			// 视频：铺满舞台、保持比例、可交互（pointer-events:auto 重新开启）、抓取光标
			// opacity:0 初始隐藏，transition 做 180ms 淡入淡出
			'.dsh-goplus-pet-video{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;pointer-events:auto;cursor:grab;opacity:0;transition:opacity .18s ease;transform-origin:center}',
			// 显示中的视频（is-front 类）
			'.dsh-goplus-pet-video.is-front{opacity:1}',
			// 按住时显示"抓取中"光标
			'.dsh-goplus-pet-video:active{cursor:grabbing}',
			...
			'@media (prefers-reduced-motion: reduce){.dsh-goplus-pet-video{transition:none}}',
		].join('\n');
		const cssTag = 'dsh-goplus-pet/style.css';
		...
		if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css="' + cssTag + '"]') === null) {
			...
			tag.dataset.plugin = 'dsh-goplus-pet';
		...
		const MOVE_LEAD_SEC = 1; // 动画开头 1s 是"准备动作"，位置不动
		const MOVE_TAIL_SEC = 1; // 动画结尾 1s 是"收尾动作"，位置不动
		...
		function Pet({ config }) {
			...
			const switchTo = (next, nextOnce) => {
				...
				if (!el) return;
				// 设置视频属性并开始加载
				el.src = '/goplus-pet/thumb/' + encodeURIComponent(next) + '.webm';
			...
			return h('div', {
				ref: rootRef,
				className: 'dsh-goplus-pet-root',
				'data-corner': corner,   // CSS 决定默认角落
				'data-facing': facing,   // CSS 决定是否镜像
				style: Object.assign({ '--dsh-goplus-pet-size': size + 'px' }, rootStyle),
				children: h('div', {
					ref: stageRef,
					className: 'dsh-goplus-pet-stage',
					style: stageStyle,
					children: [
						h('video', Object.assign({}, commonVideoProps, { ref: videoARef, className: 'dsh-goplus-pet-video is-front' })),
						h('video', Object.assign({}, commonVideoProps, { ref: videoBRef, className: 'dsh-goplus-pet-video' })),
					],
				}),
			});
		}
		...
		// ============================================================================
		const name = 'goplus-pet';        // 插件行 id（与 cordis.patch.yml 一致）
