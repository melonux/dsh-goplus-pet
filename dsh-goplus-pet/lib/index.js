/**
 * ============================================================================
 * dsh-goplus-pet 宿主半侧（host half）—— 宠物插件的"后端"部分
 * ============================================================================
 *
 * 【这个文件是什么】
...
function apply(ctx, config) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (scope !== 'thumb' && scope !== 'full') {
        ...
        res.end('dsh-goplus-pet: expected /pet/{thumb|full}/<file>');
        return;
      }
      ...
      if (file === undefined) {
        res.writeHead(400, { 'content-type': 'text/plain; charset=utf-8' });
        res.end('dsh-goplus-pet: invalid path');
        return;
      }
      ...
      if (!existsSync(file)) {
        ...
        res.end(scope === 'full'
          ? `dsh-goplus-pet: original asset not downloaded yet — run the fetch-assets script to populate ${fullRoot}`
          : 'dsh-goplus-pet: asset not found');
        return;
      }
  ...
  }), 'dsh-goplus-pet: /pet asset route');
}
